<!DOCTYPE html>
<html lang="en">
<head>
<?php include"vivekLink.php"; ?>		
</head>
<body>

<?php include"vivekHeader.php"; ?>
		
		
<div class="container">
 <div class="row">
  <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
  
     <div class="div123">
	 <br />
	 <a href="student_login.php">
	 <div class="div1">
	  <h3><i class="fa fa-hand-o-right fa-fw"></i>Login</h3>
	 </div>
	 </a>
	 <a href="online_fee.php">
	 <div class="div2">
	 <h3><i class="fa fa-hand-o-right fa-fw"></i>Online fee</h3>
	 </div>
	 </a>
	 <a href="transfer-certificate.php">
	 <div class="div3">
	 <h4><i class="fa fa-hand-o-right fa-fw"></i>Transfer-Certificate</h3> 
	 </div>
	 </a>
	 </div>
  
  </div>
  <div class="col-lg-9 col-md-9 col-sm-12 col-xs-12">
  
     <div class="R1"> </div>
     <div class="R2 table-bordered">
	    <div class="table-responsive">
		   <table class="table table-bordered table-hover table-condensed">
		   <caption> <h3>School-Time</h3></caption>
		 <thead>
		 <tr> <th style="background:#FF9900;">SEASON</th>
		   <th style="background:#FF9900;">DAY</th>
		   <th style="background:#FF9900;">LKG-UKG</th>
		   <th style="background:#FF9900;">I-XII</th> </tr>
		   </thead>
		  <tbody>
		  <tr><td>Winter</td>
		   <td>MON-SAT</td>
		   <td>9:00 am to 2:00 pm</td>
		   <td>8:30 am to 2:30 pm</td>
		   </tr>
		   <tr><td>Summer</td>
		   <td>MON-SAT</td>
		   <td>7:30 am to 12:30 pm</td>
		   <td>7:00 am to 12:30 pm</td>
		   </tr>
		   </tbody>
		   </table>
		   
		   <table class="table table-bordered table-hover table-condensed">
		   <caption> <h3>Office-Time</h3></caption>
		  <thead>
		  <tr> <th style="background:#FF9900;">SEASON</th>
		   <th style="background:#FF9900;">DAY</th>
		   <th style="background:#FF9900;">TIME</th></tr>
		   </thead>
		  <tbody>
		  <tr><td>Winter</td>
		   <td>MON-SAT</td>
		   <td>9:00 am to 2:00 pm</td>
		   </tr>
		   <tr><td>Summer</td>
		   <td>MON-SAT</td>
		   <td>7:30 am to 12:00 pm</td>
		   </tr>
		   </tbody>
		   </table>
		        <br /> <br />
        </div>
	 </div>
  
  </div>
 </div>
</div>
		
		
<?php include"vivekBottom.php" ?>
	
	
</body>
</html>
